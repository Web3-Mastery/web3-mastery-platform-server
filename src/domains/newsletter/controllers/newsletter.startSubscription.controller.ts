...
// request: POST
// route: '/api/v1/newsletter-subscription/initialize-subscription'
// access: Public
...
const createSubscription = async (req: Request<{}, ResponseSpecs, inSpecs>, res: Response<ResponseSpecs>) => {
  if (req.user) {
    ...
    try {
      ...
      if (user) {
        const newSubscriber: NewsletterSubscriberSpecs = await createNewsletterSubscription({
          subscriber: {
            email: email,
            isPlatformUser: true,
            platformUserId: user._id,
            subscriptionToken: subscriptionToken as string,
