...
// description: deletes personal(user) profile(by ID), and send back relevant data as response.
// request: DELETE
// route: '/api/v1/user/delete-personal-profile/:userId";
// access: Private
...
type ResponseSpecs = {
  ...
  response?: {
    deletedProfile: UserSpecs;
    sessionStatus?: string;
    accessToken: string;
  };
};

const deletePersonalProfile = async (req: Request<{ userId: string }, ResponseSpecs>, res: Response<ResponseSpecs>) => {
  try {
    if (req.user) {
